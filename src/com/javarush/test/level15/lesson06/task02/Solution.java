...
public class Solution {
    ...
    public static class NormalCat extends NakedCat {
        public NormalCat() {
            // конструктор по умолчанию не создатся т.к. есть кастомный, но если его создать он автомтически вызовет super()
        }

        public NormalCat(String name) {    //создается только в ручную, вызовет super()
    ...
    public static class SiamCat extends NormalCat {
        public SiamCat(String name) {      //создается в ручную
            super(name);                  //вызывает NormalCat(String name)
        }
    }
}